...
select 
  id ,
  name as 用戶名稱,
  (select sum("CREDIT_PURCHASE".purchased_credits) from "CREDIT_PURCHASE" where "CREDIT_PURCHASE".user_id = "USER".id)-
  (select count("COURSE_BOOKING".join_at) from "COURSE_BOOKING" where "COURSE_BOOKING".user_id = "USER".id) as 剩餘可用堂數
from "USER"
where "USER".name = '王小明';
...
--第二種(來自同學解答+GPT)
 ...
 select 
 	BUY.user_id,
 	(BUY.total - USED.total) as remaining_credit
 	from BUY
 	inner join USED on BUY.user_id = USED.user_id ;
...
select "CREDIT_PACKAGE".name as 方案組合,
count (*) as 銷售數量
from "CREDIT_PURCHASE"
...
from "CREDIT_PURCHASE"
where "CREDIT_PURCHASE".purchase_at >='2024-11-01 00:00:00' and "CREDIT_PURCHASE".purchase_at <='2024-12-30 23:59:59';
-- 6-5. 查詢：計算 11 月份有預約課程的會員人數（需使用 Distinct，並用 created_at 和 status 欄位統計）
-- 顯示須包含以下欄位： 預約會員人數
select count(distinct("COURSE_BOOKING".user_id)) as 預約課程的會員人數
from "COURSE_BOOKING"
